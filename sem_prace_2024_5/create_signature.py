...
if __name__ == '__main__':
    ...
    save_to_file(constants.PARAMETER_P_FILE, p)
    g = generate_random_number(500, p)
    save_to_file(constants.PARAMETER_G_FILE, g)
    x = generate_random_number(500, p)
    save_to_file(constants.PARAMETER_X_FILE, x)
    y = pow(g, x, p)
    save_to_file(constants.PARAMETER_Y_FILE, y)
    # Create signature
    student_number_hash = sha256(constants.STUDENT_NUMBER.encode('utf-8')).hexdigest()
    a, b = signature(student_number_hash, p, g)
    with open(constants.SIGNATURE_FILE, 'w') as f:
        f.write(f"{a},{b}")
        f.close()
